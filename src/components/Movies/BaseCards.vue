	...
	<div class="cardImage relative h-full w-full box-border">
		...
		<div class="text w-full h-full pb-5 absolute top-0 left-0 flex items-center justify-center flex-col">
			<div class="flex items-start justify-end h-1/2 w-full pt-2 pr-2 ">
				<p v-on:click="favorite.getFavorites.includes(movieElement?.title) ? favorite.removeItem(movieElement?.title) : favorite.addItem(movieElement?.title)"
					...
					{{ favorite.getFavorites.includes(movieElement?.title) ? "Retirer des" : "Ajouter au" }} favoris
				</p>
			</div>
